...
async def public_destinations(limit: int = Query(default=60, le=100)):
    docs = await get_db().destinations.find({}, {"_id": 0}).sort([("position", 1), ("name", 1)]).to_list(limit)
    return safe_doc(docs)


@router.get("/destinations/{slug}")
async def public_destination(slug: str):
    d = await get_db().destinations.find_one({"slug": slug}, {"_id": 0})
    if not d:
...
async def public_articles(limit: int = Query(default=60, le=100)):
    docs = await get_db().articles.find({"published": True}, {"_id": 0}).sort([("position", 1), ("published_at", -1)]).to_list(limit)
    return safe_doc(docs)


@router.get("/articles/{slug}")
async def public_article(slug: str):
    a = await get_db().articles.find_one({"slug": slug}, {"_id": 0})
    if not a:
...
async def public_packages(limit: int = Query(default=60, le=100)):
    docs = await get_db().packages.find({"active": True}, {"_id": 0}).sort([("position", 1), ("name", 1)]).to_list(limit)
    return safe_doc(docs)
